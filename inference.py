...
def inference(model, test_data_loader, result_dir, reduce_times):
    model.eval()
    ...
    for data in tqdm.tqdm(test_data_loader):
        ...
        for i in range(feat.size()[0]):
            phn_seq, transform_length = get_phn_seq(phn_boundary[i], length.cpu().numpy()[0], reduce_times)
    ...
    pk.dump(all_phn_align, open(os.path.join(result_dir, 'phn_list.pkl'), 'wb'))

def get_phn_seq(phn_boundary, length, reduce_times=2):
    interval = 2**reduce_times
    transform_length = length//interval
